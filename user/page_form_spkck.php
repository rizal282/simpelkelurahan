		...
		<?php 
			if(isset($_POST["btn-save"])){
				...
				if(!empty($ket)){
						...
						if($size_kk < 3000000){
							if($type_p_foto == "image/jpg" || $type_p_foto == "image/jpeg" || $type_p_foto == "image/png"){
								if($size_kk < 3000000){
									...
									$sql = "insert into surat values('','".$dataUser["nik"]."','".$jns."','".$n_surat."','".$ket."','','".$new_name_kk."','".$new_name_p_foto."','','Belum Verifikasi','','','".date("Y-m-d")."','','','','','')";
									...
									echo '<div class="alert alert-info">Surat Anda Telah Dibuat. Silahkan <a href="homeuser.php?pelayanan=spkck&view=v_surat">Lihat</a></div>';
								}else{
					...
					echo '<script>alert("Tolong Pilih Keperluan Surat Anda!")</script>';
				}
			}
			...
			<form method="post" action="" enctype="multipart/form-data">
				<input type="hidden" name="j_surat" value="SPKCK">
				<input type="hidden" name="n_surat" value="Surat Pengantar Keterangan Catatan Kepolisian
			">
			    <tr><div class="form-group">
			    	<label>Pilih Keperluan Pembuatan Surat Pengantar Keterangan Catatan Kepolisian: </label>
			    	</div>
			    	<div class="form-group">
			      	<input type="radio" name="ket" value="Melamar Pekerjaan"> Melamar Pekerjaan
			      	<input type="radio" name="ket" value="Kuliah"> Kuliah
			      	<input type="radio" name="ket" value="Menikah"> Menikah
			    </div>
			    <div class="form-group">
