	...
	if(!empty($ktr)){
		if(!empty($nama_kk)){
			if($type_kk == "image/jpg" || $type_kk == "image/jpeg" || $type_kk == "image/png"){
				if($size_kk < 3000000){
					$sql = "insert into surat values('','".$dataUser["nik"]."','SKTM','Surat Keterangan Tidak Mampu','".$ktr."','','".$new_name_kk."','','','Belum Verifikasi','','','".date("Y-m-d")."','','','','','')";

					move_uploaded_file($temp_kk, $lokasi.$new_name_kk);
					mysql_query($sql);

					echo '<div class="alert alert-info">Surat Anda Telah Dibuat. Silahkan <a href="homeuser.php?pelayanan=sktm&view=v_surat">Lihat</a></div>';
				}else{
...
<form method="post" action="" enctype="multipart/form-data">
   		<div class="form-group">
	    	<label>Isi Keperluan Pembuatan Surat Anda : </label>
	      	<input type="text" name="keperluan" class="form-control" placeholder="Contoh : Mendapatkan Keringan Leasing, Untuk pendaftaran Bpjs">
	    </div>
   		<div class="form-group">
